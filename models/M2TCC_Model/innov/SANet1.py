...
class SANet1(nn.Module):
    def __init__(self, gray_input=False, use_bn=True):
        super(SANet1, self).__init__()
        if gray_input:
            in_channels = 1
        else:
            in_channels = 3
        ...
        self.decoder = nn.Sequential(
            BasicConv(128, 64, kernel_size=9, use_bn=use_bn, padding=4),
            BasicConv(64, 64, kernel_size=3, use_bn=use_bn, padding=1),
            BasicConv(64, 32, kernel_size=7, use_bn=use_bn, padding=3),
            BasicConv(32, 32, kernel_size=3, use_bn=use_bn, padding=1),
            BasicConv(32, 16, kernel_size=5, use_bn=use_bn,  padding=2),
            BasicConv(16, 16, kernel_size=3, use_bn=use_bn, padding=1),
            BasicConv(16, 16, kernel_size=3, use_bn=use_bn,  padding=1),
            BasicConv(16, 1, kernel_size=1, use_bn=False),
        )
    ...
    def forward(self, x):
        ...
        up = F.upsample(out,scale_factor=8)
        return up